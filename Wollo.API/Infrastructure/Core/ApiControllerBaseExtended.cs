...
namespace Wollo.API.Infrastructure.Core
{
    public class ApiControllerBaseExtended : ApiController
    {
        ...
        protected  IEntityBaseRepository<Transaction_History> _transactionHistoryRepository;
        ...
        private void InitRepositories(List<Type> entities)
        {
            ...
            if (entities.Any(e => e.FullName == typeof(Transaction_History).FullName))
            {
                _transactionHistoryRepository = _dataRepositoryFactory.GetDataRepository<Transaction_History>(RequestMessage);
            }
        }
